...
document.getElementById('root').innerHTML =
  ...
  `<div class="page-header">
    <div><h1 class="page-title">Responses</h1><p class="page-subtitle">View and analyse individual survey responses.</p></div>
  ...
  <!-- Survey switcher -->
  <div id="survey-switcher" style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px"></div>
  ...
  <div class="card" id="resp-card">
    ...
    <div id="resp-table-wrap">
      <div class="empty-state"><div class="empty-icon"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></div><p class="empty-title">Select a survey above</p></div>
    </div>
  </div>` +
  closeSidebar();
...
async function loadSurveySwitcher() {
  ...
  if (allSurveys.length === 0) {
    sw.innerHTML = '<span style="font-size:13px;color:#94a3b8">No surveys found. <a href="surveys.html" style="color:#0079C1">Create one.</a></span>';
    return;
  }
...
async function loadResponses(sid, survey) {
  ...
  if (responses.length === 0) {
    document.getElementById('resp-table-wrap').innerHTML = '<div class="empty-state"><div class="empty-icon"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></div><p class="empty-title">No responses yet</p><p class="empty-desc">Responses will appear here once your survey is live and collecting data.</p></div>';
    return;
  }
  ...
  const rows = responses.map(resp => {
    ...
    return `<tr onclick="viewResponse('${resp.id}',${JSON.stringify(resp).replace(/"/g,'&quot;')})">
      ...
      <td><button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();viewResponse('${resp.id}',${JSON.stringify(resp).replace(/"/g,'&quot;')})">View</button></td>
    </tr>`;
  }).join('');
