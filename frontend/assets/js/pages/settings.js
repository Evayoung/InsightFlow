  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="form-group"><label class="form-label">Full Name</label><input class="form-input" id="profile-name" value="${user.full_name||''}" readonly/></div>
        <div class="form-group"><label class="form-label">Email</label><input class="form-input" id="profile-email" type="email" value="${user.email||''}" readonly/></div>
        <button class="btn btn-ghost" id="btn-save-profile">Profile Editing Not Yet Available</button>
        <div style="font-size:12px;color:#94a3b8;margin-top:8px">Account profile updates are read-only in the current API release.</div>
      </div>
    </div>
    <!-- Security -->
    <div class="card">
      ...
      <div class="card-body">
        <div class="alert alert-info">Password changes currently use the secure email recovery flow to keep the backend contract consistent.</div>
        <button class="btn btn-primary" id="btn-change-pw" style="margin-bottom:14px">Send Reset Link</button>
        <div style="font-size:12px;color:#94a3b8;margin-top:-4px;margin-bottom:14px">This sends a verified password reset link to your account email.</div>
...
document.getElementById('btn-save-profile').addEventListener('click', () => {
  toast('info', 'Read only', 'Profile editing will be enabled when the backend exposes update endpoints.');
});

document.getElementById('btn-change-pw').addEventListener('click', async () => {
  const r = await api.forgotPassword({ email: user.email });
  if (r.ok) {
    toast('success', 'Reset email sent', 'Check your inbox for the secure password reset link.');
  } else {
    toast('error', 'Unable to start reset flow', r.error.message);
  }
});
