...
async function loadDashboard() {
  if (!ws.id) {
    document.getElementById('surveys-wrap').innerHTML =
      '<div class="empty-state"><div class="empty-icon">' + icon('building') + '</div><p class="empty-title">No workspace selected</p><p class="empty-desc">Create or select a workspace to get started.</p><a href="workspaces.html" class="btn btn-primary">Go to Workspaces</a></div>';
    return;
  }
  ...
  if (!pr.ok) {
    document.getElementById('surveys-wrap').innerHTML = '<div class="empty-state"><div class="empty-icon">' + icon('clipboard') + '</div><p class="empty-title">No surveys yet</p><p class="empty-desc">Create a project and add your first survey.</p></div>';
    return;
  }

  const projects = pr.data.items || pr.data;
  if (projects.length === 0) {
    document.getElementById('surveys-wrap').innerHTML = '<div class="empty-state"><div class="empty-icon">' + icon('folder') + '</div><p class="empty-title">No projects yet</p><p class="empty-desc">Create a project to organise your surveys.</p><a href="projects.html" class="btn btn-primary">Create Project</a></div>';
    return;
  }
  ...
  if (allSurveys.length === 0) {
    document.getElementById('surveys-wrap').innerHTML = '<div class="empty-state"><div class="empty-icon">' + icon('clipboard') + '</div><p class="empty-title">No surveys yet</p><p class="empty-desc">Create your first survey to start collecting responses.</p><a href="surveys.html" class="btn btn-primary">Create Survey</a></div>';
    return;
  }
