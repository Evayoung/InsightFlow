...
document.getElementById('root').innerHTML =
  ...
  `<div class="page-header">
    ...
    <button class="btn btn-primary" onclick="openModal('modal-new-ws')">${icon('plus')} New Workspace</button>
  </div>
  <div id="ws-cards" class="grid-3" style="margin-bottom:20px"></div>
  ...
  document.getElementById('ws-cards').innerHTML = workspaces.map(w => {
    ...
    return `<div class="card" style="cursor:pointer;padding:20px;border:${active ? '2px solid #0079C1' : '1px solid #e2e8f0'}" onclick="showDetail('${w.id}')">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;flex-wrap:wrap">
        ...
        <div>
          <div style="font-family:'Plus Jakarta Sans',sans-serif;font-style:normal;font-weight:700;color:#1a2332">${w.name}</div>
          ${badge('active')}
        </div>
        ${active ? '<span style="margin-left:auto;color:#0079C1">' + icon('check') + '</span>' : ''}
      </div>
...
async function showTab(tab) {
  ...
  if (tab === 'overview') {
    ...
    el.innerHTML = `<div class="stack-grid-2">
